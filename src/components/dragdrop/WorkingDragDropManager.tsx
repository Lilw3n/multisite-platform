...
interface WorkingDragDropManagerProps {
  modules: ModuleItem[];
...
function ModuleItem({ 
  ...
  onModuleLink: (moduleId: string) => void;
  onModuleUnlink: (moduleId: string) => void;
  onModuleAdd: (parentId?: string) => void;
  parentId?: string;
}) {
  ...
  const [{ isOver, canDrop }, drop] = useDrop({
    ...
    drop: (item: DragItem) => {
      if (item.id !== module.id) {
        console.log('🎯 Drop dans module:', module.name, 'depuis:', item.id);
  ...
  const [{ isOver: isOverParent, canDrop: canDropParent }, dropParent] = useDrop({
    ...
    drop: (item: DragItem) => {
      if (item.parentId && item.parentId !== parentId) {
        console.log('⬆️ Sortie du dossier parent');
            ...
            {module.children.map((child, childIndex) => (
              <div key={child.id} className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                <ModuleItem
                  ...
                  onModuleAdd={(parentId) => onModuleAdd('project')}
                  parentId={module.id}
                />
              </div>
            ))}
...
// Composant principal
export default function WorkingDragDropManager({
  modules,
  onModulesUpdate,
  onModuleAdd,
  onModuleEdit,
  ...
  const handleModuleSelect = (moduleType: string) => {
    ...
    if (onModuleAdd) {
      ...
      console.error('onModuleAdd n\'est pas défini');
    }
    setIsSelectorOpen(false);
  };

  const moveItem = useCallback((dragIndex: number, hoverIndex: number, dragParentId?: string, hoverParentId?: string) => {
    console.log('🔄 moveItem appelé:', { dragIndex, hoverIndex, dragParentId, hoverParentId });
    ...
    if (!sourceItem) {
      console.log('❌ Source item non trouvé');
      return;
    }
            ...
            <div className="space-y-4">
              {modules.map((module, index) => (
                <ModuleItem
                  ...
                  onModuleLink={onModuleLink}
                  onModuleUnlink={onModuleUnlink}
                  onModuleAdd={(parentId) => onModuleAdd('project', parentId)}
                  parentId={undefined}
                />
              ))}
            </div>
